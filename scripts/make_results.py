...
COMMANDS = [("Unit tests", ["-m", "pytest", "-q"], False), ("Clean order clears every gate", ["-m", "tradegate", "check", "orders/example_buy.json"], False), ("Stale broker snapshot refuses the same order", ["-m", "tradegate", "check", "orders/example_buy.json", "--broker", "data/broker_stale.json"], True), ("Excluded symbol refused", ["-m", "tradegate", "check", "orders/refused_excluded.json"], True), ("Braintrust-shaped eval suite (local runner)", ["-m", "tradegate", "eval"], False), ("Overseer review: tool loop + report grounding", ["-m", "tradegate", "overseer"], False), ("Hallucinating overseer: refused", ["-m", "tradegate", "overseer", "hallucinating"], True)]
